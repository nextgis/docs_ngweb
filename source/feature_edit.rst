...
Чтобы пользователь смог вносить изменения в слои на карте, редактирование нужно включить в настройках веб-карты. Нажмите |button_edit| рядом с названием веб-карты в списке ресурсов, чтобы открыть страницу Изменение ресурса (см. :numref:`ngw_update_edit_resource_from_group_rus`).
...
*  |button_maptool_add| создать объект - активен по умолчанию при первом включении режима редактирования;
* |button_maptool_edit| редактировать;
* |button_maptool_move| переместить объект целиком;
* |button_maptool_attrib| редактировать значения атрибутов;
* |button_maptool_delete| удалить;
...
* |button_maptool_hole| вырезать отверстие.
...
3. Отроется окно ввода значений атрибутов объекта. 
...
.. note:: Если нужно создать полигон с отверстием, сначала нарисуйте внешний контур и завершите создание объекта, затем вырежьте отверстие инструментом |button_maptool_hole|.
...
.. figure:: _static/webmap_delete_polygon_ru.png
   ...
   Редактируетя слой полигонов. Полигоны, выделенные красным, останутся в слое, полигон без красного контура будет удалён
...
Чтобы передвинуть объект целиком, активируйте инструмент |button_maptool_move| переместить.

Кликните по объекту, чтобы выбрать его, затем перетащите на новое место.
...
Затем нажмите |button_maptool_save| "Завершить редактирование" и сохраните изменения.
...
На панели инструментов для редактирования нажмите кнопку |button_maptool_edit| "Редактировать".

Объекты, положение которых можно изменить, будут обвдены цветным контуром.
...
Для завершения редактирования нажмите |button_maptool_save| "Завершить редактирование" и сохраните изменения. (см. :numref:`webmap_finish_edit`).
...
Если нужно создать полигон, внутри которого вырезана одна или несколько областей. Для этого:
...
* Через таблицу объектов
  ...
  - В контекстном меню слоя выбрать "Таблица атрибутов", и далее действовать, как описано выше.
...
В открывшемся всплывающем окне можно изменять значения атрибутов, описание и вложения. 

.. note:: Если вы открыли это окно из таблицы атрибутов на отдельной странице, то также будет доступна вкладка изменения геометрию объекта. Если вы открыли редактирование атрибутов из просмотра веб-карты, то для изменения геометрии нужно активировать режим редактирования.
...
При этом, при идентификации объекта на карте в панели идентификации будет отображены как атрибуты объекта, так и вложения, которые были ему сопоставлены (см. :numref:`attachm_tab_pic`).
