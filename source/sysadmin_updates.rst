...
**Версия 3.3.0 от 27.07.2026**

* Интерактивное редактирование и настройка веб-карты в режиме отображения.
* Новая страница поиска ресурсов с расширенными возможностями фильтрации.
* Генерация фильтров объектов из запросов на естественном языке с помощью AI.
* Настройка полей при создании векторного слоя.
* Векторные слои теперь поддерживают поля булевого типа.
* Экспериментальная поддержка JSON-полей в векторных слоях.
* Ограниченная поддержка отметки полей векторного слоя как обязательных.
* Начальная поддержка слоёв без геометрии.
* Поддержка использования публичных S3-бакетов с внешними растровыми слоями.
* Улучшенный предпросмотр растрового слоя с настройкой каналов.
* Улучшен импорт CSV для справочников с предпросмотром и сопоставлением столбцов.
* Редактирование слоёв на веб-картах включено по умолчанию.
* HTTP API для импорта описаний объектов из ZIP-архивов.
* Выполнена миграция с Webpack и Yarn на Rsbuild и pnpm.
* GDAL 3.8, Python 3.12 и Node.js 22 теперь являются минимально требуемыми версиями.


**Версия 3.2.0 от 21.04.2026**

* Ресурс Галерея: новый способ организации ресурсов для пользователей.
* Поддержка фильтрации объектов на веб-картах и в HTTP API рендеринга.
* Хранилище на базе S3 для внешних растровых слоёв с поддержкой чтения и записи.
* Подсказки значений полей в фильтрах слоёв объектов.
* Поддержка атрибутов RAT при идентификации растровых слоёв.
* Версионирование объектов включено по умолчанию для новых векторных слоёв.
* Улучшен интерфейс таблицы ресурсов и действий с ними.
* Поддержка подписей в пользовательских SLD-стилях.
* Поддержка оверзума в слоях наборов тайлов.
* Автоматический выбор предиктора сжатия GeoTIFF.
* Сжатие JPEG включено по умолчанию для пирамид растровых слоёв RGB(A).
* Поддержка экспорта векторных слоёв в формате GPX.
* Ускоренный экспорт векторных слоёв с большим количеством полей.
* Поддержка заголовка Referer в соединениях WMS и TMS.
* Возможность отключить проверку TLS-сертификатов для соединений WMS.
* Поддержка удалённых СК в слоях WMS-клиента.
* Встроенный каталог СК на основе базы данных Proj.
* Поддержка значений OnlineResource в слоях WMS-клиента.
* TypeScript обновлён до версии 6.0.
* Ant Design обновлён до версии 6.2.
...
* Вкладка геометрии на страницах объектов.
...
* Убрана поддержка range-запросов GeoJSON.
...
**Версия 2.8.2 от 22.03.2025**

* Исправлена аутентификация NextGIS QGIS.

**Версия 2.8.1 от 06.03.2025**

* Исправлено переключение символов легенды.
...
**Версия 2.5.2 от 26.06.2024**

* Исправление ошибок авторизации.

**Версия 2.5.1 от 26.04.2024**

* Исправлен рендеринг категорий легенды, выключенных по-умолчанию.
...
* Автоматически генерируемые и настраиваемые легенды для веб-карт.
* Изменение порядка слоев перетаскиванием на веб-карте.
* Изменение прозрачности слоя в режиме просмотра веб-карты.
* Создание пустого векторного слоя без необходимости загрузки файла.
* Возможность заменить объекты и поля векторного слоя из файла.
* Новая таблица объектов на безе React.
* Редакторы описания объектов и ресурсов обновлены до CKEditor 5.
* Улучшена обработка описаний ресурсов в веб-картах.
* Переход к выборке объектов на веб-карте.
* Информация о свойствах в окне идентификации.
* Отображение позиции курсора и текущего охвата на веб-карте.
* Переход к охвату всех слоев веб-карты.
* Поддержка линейных и полигональных аннотаций.
* Назначаемые по умолчанию наименования ресурсов при создании.
* Быстрое удаление объектов и изменение типа геометрии векторного слоя.
* Экспорт векторных слоев по заданному охвату.
* Возможность экспорта выборки объектов векторного слоя.
* Поддержка форматов MapInfo при создании векторных слоев.
* TMS клиента: параллельная загрузка тайлов и HTTP/2.
* Удобный порядок вкладок ресурсов и их активация по умолчанию.
* Более удобное дерево слоев на веб-картах.
* Быстрое сжатие PNG при рендеринге карт и тайлов.
* Chrome 102+, Safari 15+, Edge 109+ или Firefox 102+.
* Секция прав пользователя перенесена на отдельную страницу.
* Автоматическое назначение групп на базе OAuth.
...
**Версия 1.7.3 от 08.10.2021**

* Исправлена загрузка векторного слоя с пропуском объектов с ошибками.
* Исправлена обработка ошибок при записи тайлового кэша.

**Версия 1.7.2 от 01.10.2021**

* SameSite=None в сессионной cookie для аутентификации внутри iframe.

**Версия 1.7.1 от 25.08.2021**

* Исправлена ошибка совестимости MapServer и Python 3.
* Исправлена критическая ошибка совместимости Webpack.
