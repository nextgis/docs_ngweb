...
Исходя из данных далее указываются тип геометрии, наличие/отсутствие мультигеометрий, Z-координаты и источник FID (field-ID, определять автоматически или указать из определенного поля).
На выбор представлены две кодировки - Юникод UTF-8 или Кириллица Windows-1251. Если в ESRI Shapefile присутствует файл с описанием кодировки [расширение .cpg], то он будет учитываться при загрузке. Для формата GeoJSON кодировка всегда должна быть UTF-8.
...
На вкладке **Тайловый кэш** пользователь имеет возможности задать настройки кэширования слоя (:numref:`ngweb_admin_layers_create_wms_layer_tilecache`):

* Включение
* Разрешить тайлы для запросов изображений
* Максимальный масштабный уровень
* TTL, сек (Time to live)

.. figure:: _static/admin_layers_create_wms_layer_tilecache_rus.png
   :name: ngweb_admin_layers_create_wms_layer_tilecache
   :align: center
   :width: 20cm

   Описание слоя WMS

Флаг **Разрешить тайлы для запросов изображений** позволяет клиенту запросить склейку тайлов на выверенную экранную область. **Максимальный масштабный уровень** служит пороговым значением, выше которого обращения к кэшу не происходит, изображение карты рендерится на лету. **TTL** - “время жизни” или хранения тайлов на сервере в секундах, после которого при следующем запросе изображение будет формироваться заново.
