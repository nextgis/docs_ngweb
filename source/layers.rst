...
иметь значения NULL, не должно быть полей с названиями: *id (ID), type(TYPE), source(SOURCE)*. Если вам обязательно нужны именно такие названия, то после загрузки можно переименовать в них другие поля через настройки слоя.
...
2. Выбрать систему координат, в которой следует запрашивать данные у WMS-сервера 
   (по умолчанию имеется только WGS84 / Pseudo Mercator (EPSG:3857) ).
