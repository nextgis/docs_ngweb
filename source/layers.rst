...
Вкладка TMS connection отвечает за выбор одного и настройку из способов подключения к TMS серверу - настраиваемый или геосервисы NextGIS (см. :numref:`TMS_connection_type`).
...
В случае настраиваемого способа подключения пользователь должен указать шаблон URL, параметры ключа API и используемую тайловую схему. Для геосерисов NextGIS указывается только пользовательский API-key. После заполнения всех полей нажатие кнопки *Создать* завершает процесс создания ресурса **TMS Connection**.
...
Кэширование обеспечивает повышение скорости отображения слоев веб карты, а также стилей векторных слоев. Вкладка настроек тайлового кэша состоит из следующих настроек (см. :numref:`TMS_layer_cache`):
