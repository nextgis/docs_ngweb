...
Для создания стиля необходимо сначала зайти в слой, для которого вы его создаёте. В окне свойств поля "Создать ресурс" выбрать "Стиль MapServer".
...
На вкладке Тайловый кэш при создании стиля (слоя для WMS и TMS) пользователь имеет возможности задать настройки кэширования стиля (слоя для WMS и TMS) (см. :numref:`tile_cache_settings`):
...
Опция *Разрешить тайлы для запросов изображений* позволяет клиенту запросить склейку тайлов на выверенную экранную область.
