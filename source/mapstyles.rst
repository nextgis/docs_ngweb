...
После выбора «Стиль QGIS» откроется диалог создания ресурса. В открывшемся окне на вкладке «Ресурс» необходимо внести наименование в соответствующее поле.
...
На вкладке Тайловый кэш пользователь имеет возможности задать настройки кэширования стиля (см. :numref:`tile_cache_settings`):
...
*Image compose* позволяет клиенту запросить склейку тайлов на выверенную экранную область.
*Максимальный масштабный уровень* служит пороговым значением, выше которого обращения к кэшу не происходит, изображение карты рендерится на лету.
...
Для добавления на веб-карту слоя со стилем QGIS необходимо открыть её в режиме редактирования (изменения). На вкладке **Слои** нужно добавить **стиль** соответствующего слоя и нажать *Сохранить* (см. :numref:`select_svg_style`). 
