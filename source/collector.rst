...
Если у слоёв, подключённых в проект, не было формы сбора данных, она создастся автоматически. Затем форму можно при необходимости `отредактировать <https://docs.nextgis.ru/docs_ngweb/source/collector.html#collector-create-form>`_ через диалог изменения ресурса.
...
Для того, чтобы создать новую форму в конструкторе, перетащите нужные элементы из списка слева в поле посередине. Щелкните по элементу, чтобы изменить его свойства и выбрать поле, в которое будут записываться данные. `Подробнее о каждом элементе формы <https://docs.nextgis.ru/docs_ngweb/source/form_elements.html>`_.
