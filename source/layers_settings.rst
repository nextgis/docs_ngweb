...
Редактирование векторных слоев на карте
=======================================

Введение
---------

Для редактирования основных параметров слоя необходимо `авторизоваться в Веб ГИС <https://docs.nextgis.ru/docs_ngweb/source/admin_interface.html#ngw-admin-login>`_.
...
Настройка возможности редактирования объектов на веб-карте и аннотаций
----------------------------------------------------------------------
...
3. Отроется окно ввода значений атрибутов объекта. Нажмите **ОК**, чтобы завершить создание объекта. Чтобы очистить форму, нажмите **Сбросить**.
...
Редактирование полей векторного слоя
------------------------------------

Вкладка "Поля" содержит таблицу, в которой перечислены поля (атрибуты) выбранного слоя (см. :numref:`ngweb_admin_layers_attr`). При клике на строку таблицы откроется форма редактирования атрибута. 
...
Тип поля определяет то, какие значения можно в него добавлять. Например, если вы выберете целочисленный тип, то ввести строку (текст) вы в него не сможете. Если вы не уверены какой тип выбрать - выбирайте текстовый (STRING).
